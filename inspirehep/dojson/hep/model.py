...
def add_arxiv_categories(record, blob):
    ...
    for arxiv_category in force_force_list(get_value(blob, '65017.a')):
        record['arxiv_eprints'][0]['categories'].append(arxiv_category)
...
def ensure_document_type(record, blob):
    if 'document_type' not in record:
...
def ensure_hep(record, blob):
    if 'special_collections' not in blob:
